...
const AdminComplaintsPage: React.FC = () => {
    ...
    try {
      ...
      reader.onload = (event) => {
        ...
        img.onload = () => {
          ...
          setResolvedImage(compressedDataUrl);
          toast.success("Image optimized for resolution proof!");
        };
  ...
  const handleSyncRanchiData = async () => {
    setSyncing(true);
    try {
      // 1. Trigger server seed
      try {
        ...
        console.warn('Server /api/seed call failed, proceeding with client sync:', err);
      }

      // 2. Perform client-side Firestore synchronization
      const result = await forceSyncRanchiIssuesToFirestore(true);
      if (result.success) {
        toast.success(`Ranchi municipal data synced! ${result.count || 20} issues loaded.`);
      } else {
        toast.success('Ranchi seed records updated!');
      }
    } catch (e: any) {
      console.error('Failed to sync Ranchi seed data:', e);
  ...
  useEffect(() => {
    const initialRanchiIssues: Issue[] = (RANCHI_ISSUES as any[]).map((iss, idx) => ({
      id: `seed_issue_${idx + 1}`,
      title: iss.title,
      description: iss.description,
      category: iss.category,
      severity: iss.severity || 4,
      severityReason: iss.severityReason,
      status: (iss.status || 'reported') as any,
      address: iss.address,
      reportedBy: iss.reportedBy || `citizen_rmc_${idx + 1}`,
      reporterName: iss.reporterName || `RMC Citizen Warden ${idx + 1}`,
      createdAt: { seconds: Math.floor(Date.now() / 1000) - ((idx + 1) * 7200) },
      updatedAt: { seconds: Math.floor(Date.now() / 1000) - (idx * 3600) },
      assignedDepartment: iss.category === 'pothole' ? 'Road Construction Department (RCD)' 
        : iss.category === 'streetlight' ? 'RMC Electrical & Streetlighting Cell'
        : iss.category === 'water' ? 'Drinking Water & Sanitation Department (DWSD)'
        : iss.category === 'waste' ? 'RMC Solid Waste Management Cell'
        : 'Jharkhand Bijli Vitran Nigam Limited (JBVNL)',
      imageUrl: iss.imageUrl,
      resolvedImageUrl: iss.resolvedImageUrl,
      aiTags: iss.aiTags,
      verificationReason: iss.verificationReason || ""
    }));
    ...
    const dummyIssue: Issue = {
      ...
      address: 'Near Albert Ekka Chowk, Mahatma Gandhi Main Road, Ranchi, Jharkhand 834001',
      reportedBy: 'user-rmc-001',
      reporterName: 'Sanjay Oraon',
      ...
      workerGroundReport: {
        text: "Heavy live wires scattered. Can't proceed safely without power cut from JBVNL and traffic diversion.",
        workerName: "Rahul Kumar",
        reportedAt: new Date()
      },
      autoEscalationSuggestion: "Alert: JBVNL & Ranchi Traffic Police",
      autoEscalationDepts: ["Forest Department, Ranchi Division", "JBVNL Electric Division", "Ranchi Traffic Police"]
    };

    if (!isFirebaseConfigured) {
      setIssues([dummyIssue, ...initialRanchiIssues]);
      setLoading(false);
      return;
    }
        ...
        <div style={{ marginBottom: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <button
              ...
              title="Synchronize all 20 Ranchi municipal hazard complaints"
            >
              <RefreshCw size={13} style={{ animation: syncing ? 'spin 1s linear infinite' : 'none' }} />
              <span>{syncing ? 'Syncing...' : 'Sync Ranchi Data'}</span>
            </button>
            ...
            <div style={{ border: '1px solid var(--border)', borderRadius: 6, padding: '12px', display: 'flex', flexDirection: 'column', gap: 8, background: 'var(--surface-2)' }}>
              ...
              <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 4 }}>
                ...
                >
                  ...
                  <option value="RMC Solid Waste Management Cell">RMC Solid Waste & Sanitation</option>
                  <option value="RMC Electrical & Streetlighting Cell">RMC Electrical & Streetlighting</option>
                  <option value="Drinking Water & Sanitation Department (DWSD)">Drinking Water & Sanitation Dept (DWSD)</option>
                  <option value="Jharkhand Bijli Vitran Nigam Limited (JBVNL)">JBVNL (Electric Division)</option>
                  <option value="Road Construction Department (RCD)">Road Construction Dept (RCD)</option>
                  <option value="Ranchi Traffic Police Operations">Ranchi Traffic Police Operations</option>
                  <option value="Forest Department, Ranchi Division">Forest Department (Ranchi Division)</option>
                </select>
              </div>
            </div>
                    ...
                    <div style={{ border: '1px solid #f59e0b', borderRadius: 6, padding: 12, display: 'flex', flexDirection: 'column', gap: 8, background: 'rgba(245, 158, 11, 0.05)' }}>
                      ...
                      <div style={{ marginTop: 4, paddingTop: 8, borderTop: '1px dashed rgba(245, 158, 11, 0.3)' }}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 6 }}>
                          <span style={{ fontSize: '11px', fontWeight: 600, color: 'var(--text-1)' }}>{selectedIssue.autoEscalationSuggestion || "Alert: JBVNL & Ranchi Traffic Police"}</span>
                          <button 
                            onClick={() => handleEscalateNow(selectedIssue.id, selectedIssue.autoEscalationDepts || ["Forest Department, Ranchi Division", "JBVNL Electric Division", "Ranchi Traffic Police"])}
