...
const CREWS = [
  { id: 'rmc_waste', name: 'RMC Solid Waste Management Cell', contact: '+91 651 220-0011 (Ext 124)', activeJobs: 3 },
  { id: 'dwsd_water', name: 'Drinking Water & Sanitation Dept (DWSD)', contact: '+91 651 248-1200', activeJobs: 5 },
  { id: 'jbvnl_power', name: 'JBVNL Electric Division (Central Ranchi)', contact: '+91 651 249-0520', activeJobs: 2 },
  { id: 'rcd_roads', name: 'Road Construction Dept (RCD Jharkhand)', contact: '+91 651 240-0855', activeJobs: 8 },
  { id: 'rmc_lighting', name: 'RMC Electrical & Streetlighting Cell', contact: '+91 651 220-0012', activeJobs: 2 },
  { id: 'traffic_police', name: 'Ranchi Traffic Police Operations', contact: '+91 651 221-4100', activeJobs: 1 },
];
...
const AdminAssignmentsPage: React.FC = () => {
  ...
  useEffect(() => {
    const initialIssues: Issue[] = (RANCHI_ISSUES as any[])
      .filter(i => i.status !== 'resolved')
      .map((i, idx) => ({
        id: `seed_issue_${idx + 1}`,
        title: i.title,
        category: i.category,
        severity: i.severity || 4,
        status: i.status || 'reported',
        address: i.address,
        assignedTo: i.assignedTo || undefined,
        estimatedResolutionDays: i.estimatedResolutionDays || 3,
    ...
    const unsubscribe = onSnapshot(issuesRef, (snapshot) => {
      let list = snapshot.docs
        ...
        .filter(i => !nonRanchiRegex.test(`${i.address || ''} ${i.title || ''}`));
