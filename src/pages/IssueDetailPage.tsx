    ...
    try {
      ...
      if (issue.reportedBy) {
        await createNotification(
          issue.reportedBy,
          `Notice: The reported issue "${issue.title}" has been verified as resolved. Clean municipal closing logs are now attached.`,
          issue.id
        );
      }
            ...
            <div style={{ background: '#000', position: 'relative', height: '100%', minHeight: '320px' }}>
              {issue.resolvedImageUrl ? (
                ...
                <div style={{ 
                  ...
                  {issue.category === 'streetlight' && <Lightbulb size={64} style={{ color: 'var(--text-1)' }} />}
      ...
      <div className="grid-timeline-layout" style={{ gap: '32px', alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          {issue.status !== 'resolved' && (
            <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '16px', border: '1px solid var(--border)', background: 'var(--surface-2)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <CheckCircle size={16} style={{ color: 'var(--text-1)' }} />
                <h3 style={{ fontSize: '14px', fontWeight: 600, color: 'var(--text-1)' }}>Upload Proof of Resolution</h3>
              </div>
              <p className="text-xs" style={{ color: 'var(--text-2)', margin: 0 }}>
              ...
              {resolvedImagePreview ? (
                ...
                <label style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '20px', border: '1px dashed var(--border)', borderRadius: '6px', cursor: 'pointer', transition: 'background 0.2s' }} className="hover-card">
                  <Camera size={24} style={{ color: 'var(--text-2)' }} />
                  <span style={{ fontSize: '12px', color: 'var(--text-2)', fontWeight: 500 }}>Select Proof Photo</span>
