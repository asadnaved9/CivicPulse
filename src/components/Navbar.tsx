...
export const Navbar: React.FC = () => {
  ...
  const { user, profile, userRole, loginWithGoogle, loginAnonymously, logout } = useAuth();
  const { t } = useLanguage();
      ...
      <nav className="navbar" id="app-navbar" style={{ position: 'relative' }}>
        ...
        <div className="navbar-right">
          ...
          <button 
            ...
            onClick={toggleTheme}
          >
            {theme === 'dark' ? <Sun size={16} style={{ color: 'var(--text-1)' }} /> : <Moon size={16} style={{ color: 'var(--text-2)' }} />}
          </button>
